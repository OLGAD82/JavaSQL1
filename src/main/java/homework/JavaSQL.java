    ...
    public static void main(String[] args) {
        ...
        try {
            try (Connection connection = DriverManager.getConnection(CONNECTION_URL, USER, PASSWORD)) {
                javaSQL.createCuratorTable(connection);
                javaSQL.createGroupTable(connection);
                javaSQL.createStudentTable(connection);

                final String[] curators = new String[]{"Иванов", "Петров", "Сидоров", "Федоров"};
                for (String fio : curators) {
                    javaSQL.insertDataIntoCuratorTable(connection, fio);
                }

                final String[] groups = new String[]{"IT", "ART", "SPORT"};
                int id = 1;
                for (String group : groups) {
                    javaSQL.insertDataIntoGroupTable(connection, group, id);
                    id++;
                }

                for (int i = 0; i < 15; i++) {
                    Random random = new Random();
                    javaSQL.insertDataIntoStudentTable
                            (connection, RandomStringUtils.randomAlphabetic(10), javaSQL.getRandomSex(), random.nextInt(4 - 1) + 1);
                }
                javaSQL.printStudents(connection);
                javaSQL.printCountStudents(connection);
                javaSQL.printCountStudentsFemale(connection);
                javaSQL.updateFioCurator(connection);
                javaSQL.printGroupWithCurator(connection);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
